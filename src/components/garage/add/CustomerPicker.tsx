  ...
  return (
    <div className="space-y-3">
      ...
      <div className="max-h-72 overflow-y-auto rounded-md border border-brand-border">
        {isFetching && !results ? (
          <p className="px-3 py-4 text-sm text-white/50">Načítavam…</p>
        ) : !results || results.length === 0 ? (
          <p className="px-3 py-4 text-sm text-white/50">Žiadni zákazníci.</p>
        ) : (
                  ...
                  <button
                    ...
                    className={`flex w-full items-center justify-between gap-3 px-3 py-2 text-left text-sm transition ${
                      selected
                        ...
                        : "text-white/80 hover:bg-brand-surface"
                    }`}
                  >
      ...
      {value && (
        <p className="text-xs text-white/60">
          Vybraný zákazník: <span className="text-white">{value.first_name} {value.last_name}</span>
        </p>
      )}
    </div>
  );
